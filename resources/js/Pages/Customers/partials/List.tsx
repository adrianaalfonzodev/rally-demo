...
export default function List({ agents = [] }: { agents: Agent[] }) {
  ...
  const deleteData = (dataId: number) => {
    notification.confirm(
      `¿Estás seguro de que deseas eliminar el Cliente ${dataId}? Esta acción no se puede deshacer.`,
      () => {
        post(route('administration.customers.destroy', dataId), {
          onSuccess: () =>
            notification.success('Cliente eliminado correctamente'),
          onError: () => notification.error('Error al eliminar el cliente')
        })
      }
    )
  }
  ...
  const renderActions = (data: Agent) => {
    ...
    return (
      <div className="flex space-x-4">
        <Link
          href={route('administration.customers.edit', data.id)}
